...
The persistence layer uses JPA via EclipseLink 2.7.9 (the `persistence.xml` descriptor targets the JPA 2.0 schema), mapping 12 entity classes to a MySQL backend, with two identically configured persistence units selectable at runtime. JUnit test suites cover the Verwaltung (service) and Steuerung (controller) layers.
...
- **Customer order management** — order creation, status tracking, and order history for customers
- **Warehouse and supplier domain model** — stock movements, storage locations, and inbound deliveries with line-item granularity (Lagerverkehr, Lager, Lagerort, Einlieferung, Lieferant, Lieferposition) are modelled as JPA entities; no control or GUI layer was implemented for them
- **Internal messaging system** — bidirectional messaging with status tracking (Nachricht entities)
- **Dual persistence units** — two identically configured `RESOURCE_LOCAL` units (`WAWIDBDEVPU`, `WAWIDBPRODPU`) selectable at runtime; the environment split is structural, not configurational
...
| **ComponentController** | Activation contract (`IActivateComponent`) and role enumeration (`CompType`) shared by all role components |
...
│   ├── ComponentController/       # Activation contract & role enumeration
...
├── docs/
│   ├── WAWI_Kunde_AF_Tabelle.pdf  # Use case table, Kunde domain
│   ├── WAWI_Kunde_CD.pdf          # Class diagram, Kunde domain
...
├── pom.xml                        # Maven build (fat-JAR assembly, JUnit)
...
- Two icon resources under `src/Bootloader/src/wawi/gui/bootloader/gui/resources/` are preserved as symlinks; on Windows, clone with `git clone -c core.symlinks=true …` so the login window renders its logo
